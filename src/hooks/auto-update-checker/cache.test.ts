...
describe('auto-update-checker/cache', () => {
  describe('resolveInstallContext', () => {
    test('detects OpenCode packages install root from runtime package path', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          p ===
          '/home/user/.cache/opencode/packages/blacktower@latest/package.json',
      );
      ...
      const context = resolveInstallContext(
        '/home/user/.cache/opencode/packages/blacktower@latest/node_modules/blacktower/package.json',
      );

      expect(context).toEqual({
        installDir: '/home/user/.cache/opencode/packages/blacktower@latest',
        packageJsonPath:
          '/home/user/.cache/opencode/packages/blacktower@latest/package.json',
      });
  ...
  describe('preparePackageUpdate', () => {
    ...
    test('updates packages wrapper dependency and removes installed package', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          p ===
            '/home/user/.cache/opencode/packages/blacktower@latest/package.json' ||
          p ===
            '/home/user/.cache/opencode/packages/blacktower@latest/node_modules/blacktower',
      );
      ...
      expect(result).toBe(
        '/home/user/.cache/opencode/packages/blacktower@latest',
      );
    ...
    test('keeps working when dependency is already on target version', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          ...
          p.endsWith('/.cache/opencode/node_modules/blacktower'),
      );
      ...
      const result = preparePackageUpdate('1.0.1', 'blacktower', null);
