---
name: blacktower
description: Configure and improve Blacktower for the current user. Use when users want to tune agents, models, prompts, custom agents, skills, MCPs, presets, or plugin behavior. Also use when recurring workflow friction suggests a safe config or prompt improvement.
---

# blacktower Configuration Skill
...
Blacktower is configured through a plugin config file, usually:
...
```text
~/.config/opencode/blacktower.jsonc
```
...
| `~/.config/opencode/blacktower.jsonc` | User plugin config with comments/trailing commas; takes precedence over `.json` |
| `~/.config/opencode/blacktower.json` | User plugin config generated by installer |
| `<project>/.opencode/blacktower.json` | Project-local plugin overrides checked before user config |
| `~/.config/opencode/blacktower/{agent}.md` | Full prompt replacement for a built-in agent |
| `~/.config/opencode/blacktower/{agent}_append.md` | Append-only prompt tuning for a built-in agent |
| `~/.config/opencode/blacktower/{preset}/{agent}.md` | Preset-specific full prompt replacement |
| `~/.config/opencode/blacktower/{preset}/{agent}_append.md` | Preset-specific append-only prompt tuning |
...
1. If a `preset` is active, check
   `~/.config/opencode/blacktower/{preset}/` first.
2. Fall back to `~/.config/opencode/blacktower/`.
...
- Built-in agent `prompt` and `orchestratorPrompt` fields are **not** supported
  in `blacktower.json[c]`; use markdown prompt override files instead.
...
```text
~/.config/opencode/blacktower/orchestrator_append.md
```

or for only the `openai` preset:

```text
~/.config/opencode/blacktower/openai/orchestrator_append.md
```
...
```text
~/.config/opencode/blacktower/orchestrator.md
```
...
1. **Inspect current setup**
   - Read the existing blacktower config file.
...
~/.config/opencode/blacktower/orchestrator_append.md. Apply it?
```
...
public API changes to it. Apply to your Blacktower config?
```
