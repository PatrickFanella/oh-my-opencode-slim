...
**Full written plan + review gate** — when any of:
...
3. External contracts (API, schema, migration, auth, privacy) usually
   need a full written plan and review gate.
4. If the model must remember decisions across many steps, write them
   down — that means a plan artifact.
5. Tie-breaker: unsure between direct and lightweight plan, choose
   lightweight plan. Unsure between lightweight and full, ask: 'Do we
   need a durable behavior contract?' If yes, full plan.

Before writing planning artifacts, present the artifact store policy choice:
...
When the user selects a repo-file mode (`openspec` or `hybrid`), write the
approved plan into the repo using the local convention the project already uses.
If no convention exists, prefer `docs/superpowers/plans/YYYY-MM-DD-<topic>.md`.

Once route and persistence mode are confirmed, hand off to the corresponding
planning/execution workflow. The handoff MUST include:
- `pipeline-type`: `lightweight` or `full` (matching the confirmed route)
...
- **Implementation**: direct execution after explicit approval, or a fresh implementation session if the plan is large
...
- `references/curated/brainstorming.md` — Brainstorming guidance.
- `references/curated/grill-with-docs.md` — Grill With Docs guidance.
