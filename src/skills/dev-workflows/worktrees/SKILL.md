---
name: worktrees
description: Manage Git worktrees as Blacktower safe isolated coding lanes for complex, risky, or parallel work.
---

# Worktrees Orchestration Protocol

The `worktrees` skill provides an opinionated, safe orchestration protocol for
managing Git worktrees as isolated coding lanes. Its value is giving the Orchestrator a consistent Blacktower workflow for parallel agents, risky experiments, integration review, and cleanup.
...
```text
.blacktower/worktrees/<slug>/
```

Do not create worktrees as sibling directories of the main repository.

### State Tracking (`.blacktower/worktrees.json`)

Use the optional local metadata manifest `.blacktower/worktrees.json` to maintain
structural tracking:
  ...
  "lanes": [
    {
      ...
      "branch": "blacktower/feature-auth-v2",
      "path": ".blacktower/worktrees/feature-auth-v2",
      "base": "main",
...
If `.blacktower/worktrees.json` does not exist, create it when initializing a lane,
...
- Ensure the branch name (e.g. `blacktower/<slug>` or custom project convention) does not already exist locally or on remote.
- Ensure `.blacktower/worktrees/` is ignored by Git before creating nested worktrees.
...
```gitignore
# BEGIN Blacktower worktrees
.blacktower/worktrees/
.blacktower/worktrees.json
# END Blacktower worktrees
```
...
2. Formulate a branch name. Default to `blacktower/<slug>` unless project/user conventions dictate otherwise.
...
5. Run:
   ```bash
   git worktree add -b <branch-name> .blacktower/worktrees/<slug> <base-commit/branch>
   ```
6. Register the metadata in `.blacktower/worktrees.json`.

### Phase 2: Execution & Delegation
1. Run all sub-agents with their working directory set strictly to the worktree
   path, such as `.blacktower/worktrees/<slug>`.
...
4. Safely remove the worktree using:
   ```bash
   git worktree remove .blacktower/worktrees/<slug>
   ```
5. Update `.blacktower/worktrees.json` to mark the lane as `archived` or remove it.
