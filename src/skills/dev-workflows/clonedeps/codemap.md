...
- State is trackable project metadata stored in `.blacktower/clonedeps.json`; clone
  contents live under `.blacktower/clonedeps/repos/<safe-dependency-name>/` and are
  ignored by git.
...
1. Orchestrator checks `.blacktower/clonedeps.json` first and reuses existing clones
...
5. Orchestrator writes `.blacktower/clonedeps.json` with paths, refs, and reasons.
