...
Cloned repositories live under `.blacktower/clonedeps/repos/<safe-repo-name>/`, one
folder per source repository, and are ignored by git. `.blacktower/clonedeps.json` is
...
If `.blacktower/clonedeps.json` already exists, read it first and reuse those clones
