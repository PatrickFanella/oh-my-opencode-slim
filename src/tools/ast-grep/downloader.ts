...
export function getCacheDir(): string {
  if (process.platform === 'win32') {
    ...
    return join(base, 'blacktower', 'bin');
  }
...
export async function downloadAstGrep(
  ...
  if (!platformInfo) {
    console.error(
      `[blacktower] Unsupported platform for ast-grep: ${platformKey}`,
    );
    return null;
  }
  ...
  console.log(`[blacktower] Downloading ast-grep binary...`);
  ...
  try {
    ...
    console.log(`[blacktower] ast-grep binary ready.`);

    return binaryPath;
  } catch (err) {
    console.error(
      `[blacktower] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
