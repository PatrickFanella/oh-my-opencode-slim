...
`blacktower` is an OpenCode plugin that adds a specialist-agent operating model on top of the host runtime. Its core job is to:
...
| `scripts/generate-schema.ts` | Generates `blacktower.schema.json` from the Zod config schema. |
...
| `src/control-center/` | Scheduled-task control center domain, local adapters, services, text renderer, and OpenTUI dashboard. | [View Map](src/control-center/codemap.md) |
...
| `src/skills/dev-workflows/codemap/` | Repository-mapping skill package and codemap state-management script. | [View Map](src/skills/dev-workflows/codemap/codemap.md) |
| `src/skills/dev-workflows/clonedeps/` | Workflow-only dependency source mirroring skill that routes discovery/ref resolution through librarian and direct orchestrator git operations. | [View Map](src/skills/dev-workflows/clonedeps/codemap.md) |
| `src/skills/quality-review/simplify/` | Behavior-preserving simplification skill package. | [View Map](src/skills/quality-review/simplify/codemap.md) |
...
- `.blacktower/codemap.json`: codemap change-detection state for this repository.
